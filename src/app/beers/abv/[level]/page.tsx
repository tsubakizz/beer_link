...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const option = ABV_OPTIONS.find((o) => o.value === level);
  if (!option) {
    return { title: "アルコール度数レベルが見つかりません | Beer Link" };
  }

  const title = `アルコール度数${option.label}のビール一覧`;
  const description = `アルコール度数${option.label}（${option.description}）のクラフトビールを探索。アルコール度数で選ぶビール探し。`;
...
export default async function AbvBeerPage({ params, searchParams }: Props) {
  const { level } = await params;
  const { page } = await searchParams;

  // 有効なABVレベルかチェック
  const option = ABV_OPTIONS.find((o) => o.value === level);
  if (!option || !(level in ABV_RANGES)) {
    notFound();
  }

  const range = ABV_RANGES[level as AbvLevel];
  ...
  const conditions = [
    isNotNull(beers.abv),
    gte(beers.abv, String(range.min)),
    ...(range.max !== null ? [lte(beers.abv, String(range.max))] : []),
  ];
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      {/* パンくずリスト */}
      <Breadcrumb
        items={[{ label: "ビール", href: "/beers" }, { label: `アルコール度数${option.label}` }]}
      />

      {/* ヘッダーセクション */}
      <div className="text-center mb-10">
        <h1 className="text-4xl font-bold mb-4">アルコール度数{option.label}のビール一覧</h1>
        <p className="text-lg text-base-content/70 max-w-2xl mx-auto mb-2">
          アルコール度数{option.description}のクラフトビールを探索しよう。
        </p>
        <p className="text-sm text-base-content/50">
          ABV（アルコール度数）で分類されたビールです
        </p>
      </div>
