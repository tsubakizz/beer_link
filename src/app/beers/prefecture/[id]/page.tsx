...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `${prefecture.name}のビール一覧`;
  const description = `${prefecture.name}のブルワリーが醸造するクラフトビール一覧。Beer Linkで${prefecture.name}のビールを探そう。`;
...
export default async function PrefectureBeersPage({
  params,
  searchParams,
}: Props) {
  const { id } = await params;
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="text-center mb-10">
        ...
        <Link
          href={`/breweries/prefecture/${prefecture.id}`}
      ...
      <Pagination
        ...
        basePath={`/beers/prefecture/${prefecture.id}`}
      />
    </div>
  );
}
